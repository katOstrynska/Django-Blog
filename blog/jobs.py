...
def bot(instance_id):
    ...
    if posts_with_comments == 0:
        if posts_number > bot_users_number*3:
            new_author = User.objects.create(username=''.join(random.sample(name_parts_list, 2))+str(random.randint(5, 87)), password='djangoBot', is_bot_flag=True)
            new_comment = Comment.objects.create(post_id=instance_id,
                                                 author=new_author, text='W pełni działający komentarz')
            print(new_comment)
        else:
